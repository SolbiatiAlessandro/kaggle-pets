...
class PredictiveModel(object):
    ...
    this is NB used for Multinomial Distributions, is to be used with only ONE categorical features
    ...
    def preprocess_categorical(self, X, mapping_size):
        """
        need to transform one categorical feature in to n dummies, n = 'mapping_size'
        ...
        Args:
            X: feat
            mapping_size: int, (for instance mapping_size of AdoptionSpeed is 5 = len([0,1,2,3,4])
        """
        dummies = pd.get_dummies(X, prefix='', prefix_sep='')
        dummies = dummies.T.reindex([str(n) for n in range(1, mapping_size + 1)]).T.fillna(0)
        return dummies


    def validation(self, X, Y, mapping_size, method=1, verbose=False):
        """
        ...
        for train_index, test_index in splitclass.split(X):
            ...
            self.train(train_X, train_Y,mapping_size)
    ...
    def train(self, X, Y, mapping_size, verbose=False):
        """
        train method, feature generation is inside here, data cleaning outside
        
        Args:
            X: pandas.DataFrame, only ONE CATEGORICAL VARIABLE
            mapping_size: int, (for instance mapping_size of AdoptionSpeed is 5 = len([0,1,2,3,4])
            Y: pandas.Series
        """
